...
This tool mainly focused on the deployment of security audit tools for computer systems running Linux at least. Windows security audit is an optional feature that is a project-building perspective.
...
This tool is opensource but need some competencies required. It needs to master UNIX-like platforms (Linux, MacOS) and associated tools, including software development tools (editors, interpretors, etc.). 
%It works on a
...
As Elasticsearch and Kibana are working as APIs, the frontend development part works
as a REST API (Representational State Transfer), the REST standards has to be mastered as well in order to contribute on this project.
...
\vspace{0.5cm}
Qemu is running with the support of the virtualization
...
Symfony is a PHP framework that we use to work with, which is why we chose
...
\subsection{Update about the web framework}

We finally chose to use Flask framework instead of Symfony, after further study of the necessary features:
\begin{itemize}
    \item Elegant syntax, clear namespaces (good code-maintainability in long run).
...
the hypervisor. RSA private keys must be inserted in by the system administrator within the application to allow connection to
different machines.
